"""
Content Generation Handler - Production Grade v2.0
콘텐츠 생성 기능
...
class ContentGenerationHandler(FeatureHandler):
    """
    콘텐츠 생성 핸들러 - Production Grade v2.0
    ...
    def __init__(self, brand_config: Dict[str, Any]):
        ...
        # 설정 로드
        config_dict = brand_config.get('content_generation', {})
        self.handler_config = ContentGenerationConfig(
            ...
            min_quality_score=config_dict.get('min_quality_score', 0.6),
        ...
        self.quality_filter = QualityFilter(
            config=QualityConfig(
                language="ko",
                min_length=self.handler_config.min_length,
        ...
        logger.info(
            f"[ContentGenerationHandler] Initialized for {self.brand_id} "
    ...
    def _extract_feature_config(self) -> Dict[str, Any]:
        return self.brand_config.get('content_generation', {})
    ...
    def process(self, question: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 메타데이터 초기화
        metadata = {
            'handled_by': 'content_generation',
            'handler_version': '2.0',
            'processed_at': start_time.isoformat(),
            'content_type': self._detect_content_type(question),
            'tone': self.handler_config.tone,
        }
        ...
        # 기본적으로 파이프라인에 위임 (None 반환)
        # 특정 패턴에 대해서는 직접 처리 가능
        response = self._generate_content_template(question, context)
        ...
        if response:
            ...
            if self.handler_config.validation_filter_enabled:
                ...
                if validation_result:
                    ...
                    if validation_result.get('status') == 'failed':
                        logger.warning(
                            f"[ContentGenerationHandler] Validation failed: "
            ...
            if self.handler_config.log_filter_results and all_suggestions:
                logger.info(
                    f"[ContentGenerationHandler] Suggestions: {', '.join(all_suggestions[:3])}"
                )
                metadata['improvement_suggestions'] = all_suggestions

            if self.handler_config.log_generation:
                logger.info(
                    f"[ContentGenerationHandler] Generated {metadata['content_type']} content "
                    f"({len(response)} chars) for: {question[:50]}..."
                )
    ...
    def _detect_content_type(self, question: str) -> str:
        ...
        if any(kw in question_lower for kw in ['포스트', '게시글', 'sns', '인스타']):
            return 'social_post'
        elif any(kw in question_lower for kw in ['광고', '홍보', '마케팅']):
            return 'marketing'
        elif any(kw in question_lower for kw in ['소개', '설명', '안내']):
            return 'introduction'
        elif any(kw in question_lower for kw in ['이메일', '메일']):
            return 'email'
        else:
            return 'general'
    ...
    def _apply_quality_filter(
        ...
        try:
            ...
            if self.handler_config.log_filter_results:
                logger.info(
                    f"[ContentGenerationHandler:Quality] "
            ...
            logger.error(f"[ContentGenerationHandler:Quality] Error: {e}")
            return None
    ...
    def _apply_trust_filter(
        ...
        try:
            ...
            if self.handler_config.log_filter_results:
                logger.info(
                    f"[ContentGenerationHandler:Trust] "
            ...
            logger.error(f"[ContentGenerationHandler:Trust] Error: {e}")
            return None
    ...
    def _apply_relevance_filter(
        ...
        try:
            ...
            if self.handler_config.log_filter_results:
                logger.info(
                    f"[ContentGenerationHandler:Relevance] "
                    f"Score={result.score:.2f}, "
                ...
                if result.issues:
                    for issue in result.issues[:3]:
                        logger.warning(
                            f"[ContentGenerationHandler:Relevance] Issue: "
            ...
            logger.error(f"[ContentGenerationHandler:Relevance] Error: {e}")
            return None
    ...
    def _apply_validation_filter(
        ...
        try:
            ...
            if self.handler_config.log_filter_results:
                logger.info(
                    f"[ContentGenerationHandler:Validation] "
                    f"Score={result.score:.2f}, "
                ...
                if result.all_issues:
                    for issue in result.all_issues[:3]:
                        logger.warning(f"[ContentGenerationHandler:Validation] Issue: {issue}")
            ...
            logger.error(f"[ContentGenerationHandler:Validation] Error: {e}")
            return None
